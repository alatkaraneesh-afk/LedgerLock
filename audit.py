...
def run_audit(df):
    ...
    def simplify_vendor(v):
        v = str(v).lower()
        if 'aws' in v or 'amazon' in v or 'amzn' in v:
            return 'amazon/aws'
        if 'acme' in v:
    ...
    for i, row_a in df.iterrows():
        if row_a['row_id'] in flagged_ids:
            continue

        for j, row_b in df.iterrows():
            if i >= j or row_b['row_id'] in flagged_ids:
                continue

            same_amt = abs(row_a['amount'] - row_b['amount']) < 0.01
            name_match = (
                (row_a['vendor_group'] == row_b['vendor_group']) or
                (fuzz.token_set_ratio(row_a['vendor'], row_b['vendor']) > 85)
            )
...
def generate_pdf_report(findings, waste):
    ...
    for _, row in findings.iterrows():
        ...
        pdf.cell(75, 10, str(row['vendor'])[:40], 1)
...
if uploaded_file:
    ...
    if st.button("🚀 Run Forensic Audit"):
        ...
        if not findings.empty:
            ...
            st.download_button(
                "📥 Download Certified Forensic Audit",
                data=pdf_bytes,
                file_name="LedgerLock_Report.pdf",
            ...
            for i, row in findings.iterrows():
                ...
                if row['amount'] > 1000:
                    risk_score += 40
                if "Match" in row['issue'] or "DUPLICATE" in row['issue']:
                    risk_score += 30
                if "SPIKE" in row['issue']:
                    risk_score += 30
            ...
            st.success("✅ No financial leakage detected. This company ledger is lean.")import streamlit as st
from supabase import create_client, Client
