...
def run_audit(df):
    # Standardize column names to prevent "Header Errors"
    df.columns = df.columns.str.lower().str.strip()
    
    # Data Normalization: Force dates and numbers to be correct
    df['date'] = pd.to_datetime(df['date'], errors='coerce')
    df['amount'] = pd.to_numeric(df['amount'], errors='coerce')
    df['vendor_clean'] = df['vendor'].str.lower().str.strip()
    
    # ALGORITHM 1: Exact Duplicate Detection
    # Finds rows with the same Vendor, Amount, and Date
    dupes = df[df.duplicated(subset=['vendor_clean', 'amount', 'date'], keep=False)]
    
    # ALGORITHM 2: Price Escalation Detection (>20% jump)
    df = df.sort_values(['vendor_clean', 'date'])
    df['prev_amt'] = df.groupby('vendor_clean')['amount'].shift(1)
